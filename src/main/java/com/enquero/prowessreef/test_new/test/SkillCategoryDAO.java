 ...
 * SkillCategory entities. Transaction control of the save(), update() and
 * delete() operations must be handled externally by senders of these methods or
 * must be manually added to each of these methods for data to be persisted to
 * the JPA datastore.
 * 
 * @see com.test.SkillCategory
 * @author MyEclipse Persistence Tools
 */
public class SkillCategoryDAO implements ISkillCategoryDAO {
	// property constants
	public static final String SKILL_CATEGORY_NAME = "skillCategoryName";
	...
	public void save(SkillCategory entity) {
		EntityManagerHelper.log("saving SkillCategory instance", Level.INFO, null);
		try {
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * SkillCategoryDAO.delete(entity);
	 * EntityManagerHelper.commit();
	 * entity = null;
	 * </pre>
	 * 
	...
	public void delete(SkillCategory entity) {
		EntityManagerHelper.log("deleting SkillCategory instance", Level.INFO, null);
		try {
			entity = getEntityManager().getReference(SkillCategory.class, entity.getId());
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * entity = SkillCategoryDAO.update(entity);
	 * EntityManagerHelper.commit();
	 * </pre>
	 * 
	...
	public SkillCategory update(SkillCategory entity) {
		EntityManagerHelper.log("updating SkillCategory instance", Level.INFO, null);
		try {
			SkillCategory result = getEntityManager().merge(entity);
	...
	public SkillCategory findById(Integer id) {
		EntityManagerHelper.log("finding SkillCategory instance with id: " + id, Level.INFO, null);
		try {
			SkillCategory instance = getEntityManager().find(SkillCategory.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<SkillCategory> findByProperty(String propertyName, final Object value,
			final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding SkillCategory instance with property: " + propertyName + ", value: " + value,
				Level.INFO, null);
		try {
			final String queryString = "select model from SkillCategory model where model." + propertyName
					+ "= :propertyValue";
	...
	public List<SkillCategory> findBySkillCategoryName(Object skillCategoryName, int... rowStartIdxAndCount) {
		return findByProperty(SKILL_CATEGORY_NAME, skillCategoryName, rowStartIdxAndCount);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<SkillCategory> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all SkillCategory instances", Level.INFO, null);
		try {
			final String queryString = "select model from SkillCategory model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
